...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id E001B34EDDD
	for <lists+linux-ext4@lfdr.de>; Tue, 30 Mar 2021 18:31:03 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S231743AbhC3Qa3 (ORCPT <rfc822;lists+linux-ext4@lfdr.de>);
        Tue, 30 Mar 2021 12:30:29 -0400
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:43608 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S231627AbhC3Q34 (ORCPT
        <rfc822;linux-ext4@vger.kernel.org>); Tue, 30 Mar 2021 12:29:56 -0400
Received: from mail-pg1-x535.google.com (mail-pg1-x535.google.com [IPv6:2607:f8b0:4864:20::535])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id 1428DC061574
        for <linux-ext4@vger.kernel.org>; Tue, 30 Mar 2021 09:29:56 -0700 (PDT)
Received: by mail-pg1-x535.google.com with SMTP id y32so10950126pga.11
        for <linux-ext4@vger.kernel.org>; Tue, 30 Mar 2021 09:29:56 -0700 (PDT)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed;
        d=dilger-ca.20150623.gappssmtp.com; s=20150623;
        h=content-transfer-encoding:from:mime-version:subject:date:message-id
         :references:cc:in-reply-to:to;
        bh=7v+dBGry3yURNTRcNicBFDIcZ2eauXuOJT/vUUHatsk=;
        b=
X-Google-DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed;
        ...
        bh=7v+dBGry3yURNTRcNicBFDIcZ2eauXuOJT/vUUHatsk=;
        b=
X-Gm-Message-State: AOAM533vPCxs5whG1j6+iStORPwfNjSZY4OJU2jRJDnmru/nM6DV7i3k
        3/WBrg1i02EBcJ6qc2+8Gi6j/7tXMQgP9Pob
X-Google-Smtp-Source: ABdhPJwnIQzxO+BoE8OuRhO6cCrA2gje3SG90uygwWdUT0zr4qMuo5xMg8JvXxM+OPNz1fqLA/wYMw==
X-Received: by 2002:a63:925a:: with SMTP id s26mr15596755pgn.216.1617121794826;
        Tue, 30 Mar 2021 09:29:54 -0700 (PDT)
Received: from [192.168.10.175] (S01061cabc081bf83.cg.shawcable.net. [70.77.221.9])
        by smtp.gmail.com with ESMTPSA id m7sm21229475pfd.52.2021.03.30.09.29.53
...
Date:   Tue, 30 Mar 2021 09:29:52 -0700
Message-Id: <E70919BD-0CA3-4584-A939-3C8A467039EB@dilger.ca>
References: <CAD+ocbyhQEtt8EDdS=vtK8YLiKCSuvzTMZX2DHz4RQ_nfdv-7g@mail.gmail.com>
Cc:     Ext4 Developers List <linux-ext4@vger.kernel.org>,
        kernel test robot <lkp@intel.com>,
        Dan Carpenter <dan.carpenter@oracle.com>,
        Alex Zhuravlev <bzzz@whamcloud.com>
In-Reply-To: <CAD+ocbyhQEtt8EDdS=vtK8YLiKCSuvzTMZX2DHz4RQ_nfdv-7g@mail.gmail.com>
...
On Mar 30, 2021, at 09:20, harshad shirwadkar <harshadshirwadkar@gmail.com> w=
rote:
>=20
> =EF=BB=BFI ran some experiments this morning and here are the results:
>=20
> 1) Config 1:
...
>>=20
>>> On Mar 29, 2021, at 17:03, harshad shirwadkar <harshadshirwadkar@gmail.c=
om> wrote:
>>>=20
>>> =EF=BB=BFAnother problem that I found in the patch was that when all gro=
ups are
...
>>>>=20
>>>>> On Mon, Mar 29, 2021 at 12:14 PM Andreas Dilger <adilger@dilger.ca> wr=
ote:
>>>>>=20
>>>>> On Mar 24, 2021, at 5:19 PM, Harshad Shirwadkar <harshadshirwadkar@gma=
il.com> wrote:
>>>>>>=20
>>>>>> Instead of traversing through groups linearly, scan groups in specifi=
c
>>>>>> orders at cr 0 and cr 1. At cr 0, we want to find groups that have th=
e
...
>>>>>> allocation, we traverse these lists in the increasing order of larges=
t
>>>>>> free orders. This allows us to find a group with the best available c=
r
...
>>>>>> of traversing linearly, at CR1, we traverse in the order of increasin=
g
>>>>>> average fragment size, starting at the most optimal group. This bring=
s
...
>>>>>=20
>>>>> If you are refreshing this patch anyway, could you rename "mb_linear_l=
imit"
>>>>> to "mb_linear_groups" or "mb_max_linear_groups" or similar?  It otherw=
ise
>>>>> isn't clear what units the "limit" is in (could be MB/GB or something e=
lse).
>>>>>=20
>>>>> Cheers, Andreas
>>>>>=20
...
>>>>>> blocks) is greater than the request extent size. That's left as futur=
e
>>>>>> work.
>>>>>>=20
...
>>>>>> #define EXT4_MOUNT2_JOURNAL_FAST_COMMIT       0x00000010 /* Journal f=
ast commit */
>>>>>> #define EXT4_MOUNT2_DAX_NEVER         0x00000020 /* Do not allow Dire=
ct Access */
>>>>>> #define EXT4_MOUNT2_DAX_INODE         0x00000040 /* For printing opti=
ons only */
>>>>>> -
...
>>>>>> *
>>>>>> + * If "mb_optimize_scan" mount option is set, we maintain in memory g=
roup info
>>>>>> + * structures in two data structures:
>>>>>> + *
>>>>>> + * 1) Array of largest free order lists (sbi->s_mb_largest_free_orde=
rs)
>>>>>> + *
>>>>>> + *    Locking: sbi->s_mb_largest_free_orders_locks(array of rw locks=
)
>>>>>> + *
>>>>>> + *    This is an array of lists where the index in the array represe=
nts the
>>>>>> + *    largest free order in the buddy bitmap of the participating gr=
oup infos of
>>>>>> + *    that list. So, there are exactly MB_NUM_ORDERS(sb) (which mean=
s total
>>>>>> + *    number of buddy bitmap orders possible) number of lists. Group=
-infos are
>>>>>> + *    placed in appropriate lists.
>>>>>> + *
>>>>>> + * 2) Average fragment size rb tree (sbi->s_mb_avg_fragment_size_roo=
t)
>>>>>> + *
>>>>>> + *    Locking: sbi->s_mb_rb_lock (rwlock)
>>>>>> + *
>>>>>> + *    This is a red black tree consisting of group infos and the tre=
e is sorted
>>>>>> + *    by average fragment sizes (which is calculated as ext4_group_i=
nfo->bb_free
>>>>>> + *    / ext4_group_info->bb_fragments).
>>>>>> + *
>>>>>> + * When "mb_optimize_scan" mount option is set, mballoc consults the=
 above data
>>>>>> + * structures to decide the order in which groups are to be traverse=
d for
>>>>>> + * fulfilling an allocation request.
>>>>>> + *
>>>>>> + * At CR =3D 0, we look for groups which have the largest_free_order=
 >=3D the order
>>>>>> + * of the request. We directly look at the largest free order list i=
n the data
>>>>>> + * structure (1) above where largest_free_order =3D order of the req=
uest. If that
>>>>>> + * list is empty, we look at remaining list in the increasing order o=
f
...
>>>>>> + * At CR =3D 1, we only consider groups where average fragment size >=
 request
>>>>>> + * size. So, we lookup a group which has average fragment size just a=
bove or
>>>>>> + * equal to request size using our rb tree (data structure 2) in O(l=
og N) time.
>>>>>> + *
>>>>>> + * If "mb_optimize_scan" mount option is not set, mballoc traverses g=
roups in
>>>>>> + * linear order which requires O(N) search time for each CR 0 and CR=
 1 phase.
>>>>>> + *
>>>>>> * The regular allocator (using the buddy cache) supports a few tunabl=
es.
>>>>>> *
...
>>>>>> *
>>>>>> * The regular allocator uses buddy scan only if the request len is po=
wer of
>>>>>> * 2 blocks and the order of allocation is >=3D sbi->s_mb_order2_reqs.=
 The
>>>>>> @@ -149,6 +188,16 @@
>>>>>> * can be used for allocation. ext4_mb_good_group explains how the gro=
ups are
>>>>>> * checked.
>>>>>> *
>>>>>> + * When "mb_optimize_scan" is turned on, as mentioned above, the gro=
ups may not
...
>>>>>> + * close to each other. And so, the underlying device may get filled=
 up in a
>>>>>> + * non-linear fashion. While that may not matter on non-rotational d=
evices, for
>>>>>> + * rotational devices that may result in higher seek times. "mb_line=
ar_limit"
>>>>>> + * tells mballoc how many groups mballoc should search linearly befo=
re
>>>>>> + * performing consulting above data structures for more efficient lo=
okups. For
>>>>>> + * non rotational devices, this value defaults to 0 and for rotation=
al devices
>>>>>> + * this is set to MB_DEFAULT_LINEAR_LIMIT.
>>>>>> + *
>>>>>> * Both the prealloc space are getting populated as above. So for the f=
irst
>>>>>> * request we will hit the buddy cache which will result in this preal=
loc
...
>>>>>> * 1. We sample the percpu discard_pa_seq counter before trying for bl=
ock
>>>>>> @@ -744,6 +801,251 @@ static void ext4_mb_mark_free_simple(struct sup=
er_block *sb,
>>>>>>     }
>>>>>> }
>>>>>>=20
...
>>>>>> +
>>>>>> +static int
>>>>>> +ext4_mb_avg_fragment_size_cmp(struct rb_node *rb1, struct rb_node *r=
b2)
>>>>>> +{
...
>>>>>> + */
>>>>>> +static void
>>>>>> +mb_update_avg_fragment_size(struct super_block *sb, struct ext4_grou=
p_info *grp)
>>>>>> +{
...
>>>>>> + * Choose next group by traversing largest_free_order lists. Return 0=
 if next
>>>>>> + * group was selected optimally. Return 1 if next group was not sele=
cted
>>>>>> + * optimally. Updates *new_cr if cr level needs an update.
>>>>>> + */
>>>>>> +static int ext4_mb_choose_next_group_cr0(struct ext4_allocation_cont=
ext *ac,
...
>>>>>> +             return 1;
>>>>>> +
>>>>>> +     if (unlikely(sbi->s_mb_stats && ac->ac_flags & EXT4_MB_CR0_OPTI=
MIZED))
...
>>>>>> +             if (list_empty(&sbi->s_mb_largest_free_orders[i])) {
>>>>>> +                     read_unlock(&sbi->s_mb_largest_free_orders_lock=
s[i]);
>>>>>> +                     continue;
>>>>>> +             }
>>>>>> +             grp =3D NULL;
>>>>>> +             list_for_each_entry(iter, &sbi->s_mb_largest_free_order=
s[i],
>>>>>> +                                 bb_largest_free_order_node) {
>>>>>> +                     if (sbi->s_mb_stats)
>>>>>> +                             atomic64_inc(&sbi->s_bal_cX_groups_cons=
idered[0]);
>>>>>> +                     if (likely(ext4_mb_good_group(ac, iter->bb_grou=
p, 0))) {
>>>>>> +                             grp =3D iter;
>>>>>> +                             break;
>>>>>> +                     }
...
>>>>>> +
>>>>>> +/*
>>>>>> + * Choose next group by traversing average fragment size tree. Retur=
n 0 if next
>>>>>> + * group was selected optimally. Return 1 if next group could not se=
lected
>>>>>> + * optimally (due to lock contention). Updates *new_cr if cr lvel ne=
eds an
>>>>>> + * update.
>>>>>> + */
>>>>>> +static int ext4_mb_choose_next_group_cr1(struct ext4_allocation_cont=
ext *ac,
>>>>>> +             int *new_cr, ext4_group_t *group, ext4_group_t ngroups)=
...
>>>>>> +
>>>>>> +     /*
>>>>>> +      * If there is contention on the lock, instead of waiting for t=
he lock
>>>>>> +      * to become available, just continue searching lineraly. We'll=
 resume
>>>>>> +      * our rb tree search later starting at ac->ac_last_optimal_gro=
up.
>>>>>> +      */
...
>>>>>> +             grp =3D ext4_get_group_info(ac->ac_sb, ac->ac_last_opti=
mal_group);
>>>>>> +             for (found =3D rb_next(&grp->bb_avg_fragment_size_rb); f=
ound !=3D NULL;
...
>>>>>> +                     if (!best_so_far || avg_fragment_size < best_so=
_far) {
...
>>>>>> + * @new_cr    This is an output parameter. If the there is no good g=
roup
>>>>>> + *            available at current CR level, this field is updated t=
o indicate
>>>>>> + *            the new cr level that should be used.
>>>>>> + * @group     This is an input / output parameter. As an input it in=
dicates the
>>>>>> + *            last group used for allocation. As output, this field i=
ndicates
...
>>>>>> +
>>>>>> /*
>>>>>> * Cache the order of the largest free extent we have available in thi=
s block
>>>>>> * group.
>>>>>> @@ -751,18 +1053,33 @@ static void ext4_mb_mark_free_simple(struct su=
per_block *sb,
>>>>>> static void
>>>>>> mb_set_largest_free_order(struct super_block *sb, struct ext4_group_i=
nfo *grp)
>>>>>> {
>>>>>> +     struct ext4_sb_info *sbi =3D EXT4_SB(sb);
>>>>>>     int i;
>>>>>> -     int bits;
>>>>>>=20
>>>>>> +     if (test_opt2(sb, MB_OPTIMIZE_SCAN) && grp->bb_largest_free_ord=
er >=3D 0) {
>>>>>> +             write_lock(&sbi->s_mb_largest_free_orders_locks[
>>>>>> +                                           grp->bb_largest_free_orde=
r]);
...
>>>>>> +                   &sbi->s_mb_largest_free_orders[grp->bb_largest_fr=
ee_order]);
>>>>>> +             write_unlock(&sbi->s_mb_largest_free_orders_locks[
>>>>>> +                                           grp->bb_largest_free_orde=
r]);
...
>>>>>>     mb_check_buddy(e4b);
>>>>>>=20
>>>>>> @@ -2347,17 +2667,21 @@ ext4_mb_regular_allocator(struct ext4_allocat=
ion_context *ac)
>>>>>>              * from the goal value specified
>>>>>>              */
>>>>>>             group =3D ac->ac_g_ex.fe_group;
>>>>>> +             ac->ac_last_optimal_group =3D group;
>>>>>> +             ac->ac_groups_linear_remaining =3D sbi->s_mb_linear_lim=
it;
...
>>>>>>                     cond_resched();
>>>>>> -                     /*
>>>>>> -                      * Artificially restricted ngroups for non-exte=
nt
>>>>>> -                      * files makes group > ngroups possible on firs=
t loop.
>>>>>> -                      */
>>>>>> -                     if (group >=3D ngroups)
>>>>>> -                             group =3D 0;
>>>>>> +
>>>>>> +                     ext4_mb_choose_next_group(ac, &new_cr, &group, n=
groups);
>>>>>> +
>>>>>> +                     if (new_cr !=3D cr) {
>>>>>> +                             cr =3D new_cr;
...
>>>>>> +             kmalloc_array(MB_NUM_ORDERS(sb), sizeof(struct list_hea=
d),
>>>>>> +                     GFP_KERNEL);
...
>>>>>>=20
>>>>>> +/*
>>>>>> + * Number of groups to search linearly before performing group scann=
ing
>>>>>> + * optimization.
>>>>>> + */
...
>>>>>> +
>>>>>> +/*
>>>>>> + * Minimum number of groups that should be present in the file syste=
m to perform
...
>>>>>> @@ -2092,6 +2096,7 @@ static int ext4_set_test_dummy_encryption(struc=
t super_block *sb,
...
>>>>>> };
>>>>>>=20
>>>>>> static int handle_mount_opt(struct super_block *sb, char *opt, int to=
ken,
...
>>>>>> +                              "mb_optimize_scan should be set to 0 o=
r 1.");
>>>>>> +                     return -1;
>>>>>> +             }
...
>>>>>> +
>>>>>> +     /*
>>>>>> +      * Enable optimize_scan if number of groups is > threshold. Thi=
s can be
>>>>>> +      * turned off by passing "mb_optimize_scan=3D0". This can also b=
e
...
>>>>>> +     else if (sbi->s_groups_count >=3D MB_DEFAULT_LINEAR_SCAN_THRESH=
OLD)
>>>>>> +             set_opt2(sb, MB_OPTIMIZE_SCAN);
>>>>>> +
...
>>>>>> --- a/fs/ext4/sysfs.c
>>>>>> +++ b/fs/ext4/sysfs.c
>>>>>> @@ -213,6 +213,7 @@ EXT4_RW_ATTR_SBI_UI(mb_order2_req, s_mb_order2_re=
qs);
